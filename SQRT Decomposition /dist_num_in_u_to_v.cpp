...
public:
    int l,r,id,lca;
    dat(){};
    dat(int l,int r,int id,int lca)
    {
        this->l=l;
        this->r=r;
        this->id=id;
...
void solve()
{
    ...
    sort(all(v));
    int un=unique(all(v))-v.begin();
    while(v.size()>un) v.pop_back();
    for(int i=0;i<n;i++)
    {
        vec[i]=1+lower_bound(all(v),vec[i])-v.begin();
    }
  ...
  auto lca=[&](int u,int v)
  {
    ...
    int dis=0;
    vector<int> freq(un+5,0),vis(n+5,0);
