...
void solve()
{
    ...
    vector<int> mn(block,1e9);
    for(int i=0;i<block;i++)
    {
        int l=i*block_size;
        int r=min(n,block_size*(i+1));
        int cur=l/block_size;
        for(int j=l;j<r;j++)
        {
            mn[cur]=min(mn[cur],vec[j]);
        }
    }
 
    auto change=[&](int l,int u)
    {
        int block_l=l/block_size;
        mn[block_l]=1e9;
        vec[l]=u;
        int x=(block_l+1)*block_size;
        for(int i=block_l*block_size;i<x;i++)
        {
            mn[block_l]=min(mn[block_l],vec[i]);
        }
    };
